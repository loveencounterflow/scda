...
create table ${this._schema_i}.occurrences (
  ...
  primary key ( spath, lnr, cnr ) );`);
    ...
    //---------------------------------------------------------------------------------------------------------
    _add_dependencies() {
      var consumer_spath, i, idx, len, provider_spath, ref;
      ref = this.cfg.dependencies;
      for (idx = i = 0, len = ref.length; i < len; idx = ++i) {
        provider_spath = ref[idx];
        if ((consumer_spath = this.cfg.dependencies[idx + 1]) == null) {
          continue;
        }
        this.dba.run(`insert into ${this._schema_i}.dependencies ( provider_spath, consumer_spath )
  values ( $provider_spath, $consumer_spath );`, {provider_spath, consumer_spath});
      }
      return null;
    }
